...
const EventsPage = () => {
    ...
    const fetchEvents = useCallback(async () => {
        setLoading(true);
        try {
            // Fetch all events with page, size, and filters
            const result = await searchEvents('', filters, page, EVENTS_PER_PAGE);
            ...
            <main className="py-8 px-4 sm:px-6 lg:px-8">
                <div className="max-w-7xl mx-auto bg-[#151f1b] rounded-2xl border border-[#2f3d37] p-5 shadow-lg">
                    <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <h1 className="text-2xl font-bold">Tất cả sự kiện</h1>
                            <p className="text-sm text-gray-300 mt-1">
                                {loading ? 'Đang tải...' : `${totalElements} sự kiện được tìm thấy`}
                            </p>
                        </div>
                    ...
                    {loading ? (
                        ...
                        <div className="mt-12 text-center text-gray-400">
                            <p className="text-lg">Không tìm thấy sự kiện nào.</p>
                        </div>
                    ) : (
