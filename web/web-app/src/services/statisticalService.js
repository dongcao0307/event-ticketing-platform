...
export const getRevenueStats = async (startDate, endDate) => {
  try {
    const response = await get('/admin/statistics/revenue', {
...
export const getTopCustomers = async () => {
  try {
    const response = await get('/admin/statistics/top-customers');
    return response.data;
  } catch (error) {
...
export const getEventStats = async (eventId, startDate, endDate) => {
  try {
    const response = await get(`/admin/statistics/events/${eventId}`, {
      start: startDate, end: endDate 
    });
    return response.data;
  } catch (error) {
