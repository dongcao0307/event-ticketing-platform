...
import { X, Lock, Unlock, Mail, Phone, Calendar, ShoppingCart, CalendarDays } from 'lucide-react';
...
const formatPhone = (phone) => {
  ...
  return '+84 ' + value.slice(1, 4) + ' ' + value.slice(4, 7) + ' ' + value.slice(7);
};
...
const UserDetailModal = ({ user, onClose, onAction }) => {
  const key = user?.userName || user?.id || user?.email || user?.name || 'user';
  const eventCount  = (String(key).length * 3) % 19 + 1;
  const orderCount  = (String(key).length * 7) % 24 + 1;
  const avatarColor = user.avatarColor || getAvatarColor(key);
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center pt-6 pb-5 px-5">
          ...
          {user.userName && <p className="text-gray-500 text-xs mt-1">@{user.userName}</p>}
        ...
        <div className="px-5 pb-4">
          ...
          <div className="space-y-3">
            ...
            {user.role && (
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-lg bg-[#2a2a2a] flex items-center justify-center shrink-0"><CalendarDays size={13} className="text-gray-500" /></div>
                <div>
                  <p className="text-gray-500 text-[11px]">Vai trò</p>
            ...
            {user.city && (
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-lg bg-[#2a2a2a] flex items-center justify-center shrink-0"><CalendarDays size={13} className="text-gray-500" /></div>
                <div>
                  <p className="text-gray-500 text-[11px]">Thành phố</p>
                  <p className="text-gray-200 text-sm">{user.city}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="px-5 pb-5">
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-white text-sm font-medium transition-colors ${isActive ? 'bg-red-500 hover:bg-red-600' : 'bg-[#26bc71] hover:bg-[#1ea860]'}`}
          >
